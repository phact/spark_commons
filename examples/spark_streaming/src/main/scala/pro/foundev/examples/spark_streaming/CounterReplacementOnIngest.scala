...
import pro.foundev.examples.spark_streaming.logging.{LogDetail, Log}
...
object CounterReplacementOnIngest extends CassandraCapable{
  ...
  def main(args: Array[String]): Unit = {
    ...
    val kafkaStream = KafkaUtils.createStream(context.streamingContext, host, "consumer-group", topicMap);
    kafkaStream.map(
        message => {
      ...
      connector.withSessionDo(session=>{
        ...
        val safeUpdate = (param:String, updated: Long, previous: Long) => {
          session.execute(s"UPDATE ${keySpaceName}.${logs} set ${param}_count = ? WHERE service_name = ? IF ${param}_count = ?"
            ,updated: _root_.java.lang.Long , newLogDetail.serviceName, previous: _root_.java.lang.Long
          )
